...
function alvoExplicacaoLinha() {
  ...
  if (proxima !== null)
    return { indice: proxima, status: traduzir("status.running") };
  const executada = linhaExecutadaAtual();
  if (executada !== null)
    return { indice: executada, status: traduzir("status.executed") };
  return null;
}
...
function atualizarExplicacaoLinha() {
  ...
  corpo.innerHTML =
    ...
    '<span class="explicacao-chip">' +
    escaparHTML(traduzir("input.line")) +
    " " +
    identificadorLinha +
    "</span>" +
    '<span class="explicacao-chip">' +
    escaparHTML(alvo.status) +
    "</span>" +
    "</div>" +
